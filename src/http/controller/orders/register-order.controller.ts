...
export async function RegisterOrderController(req: FastifyRequest, res: FastifyReply) {
    ...
    const { customer_id, store_id } = RegisterOrderBodySchema.parse(req.body)
    ...
    const { order, error } = await registerOrderService.execute({ storeId: store_id, customerId: customer_id,  });
    if (error) {
